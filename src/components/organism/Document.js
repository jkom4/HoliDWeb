...
export default function Document({ item }) {
    ...
    const handleUploadClick = async () => {
        if (selectedFile) {
            ...
            try {
                setLoading(true);
                await API.uploadDocument(formData, item.id);
                setSelectedFile(null); // Réinitialiser la sélection du fichier
    ...
    const handleDownload = async (filename) => {
        try {
            const response =  await API.trackDocumentDownload(item.id,filename); // Appel API pour enregistrer le téléchargement
